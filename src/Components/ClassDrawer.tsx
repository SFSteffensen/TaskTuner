...
import { createICalendarEvents } from '../Util/createCalendarEvents';
...
function ClassDrawer(props: ClassModalProps) {
  ...
  const [translateX, setTranslateX] = createSignal(
    isModalOpen() ? TRANSLATE_X_ZERO : TRANSLATE_X_FULL
  );
  ...
  const downloadClassSchedule = async () => {
    if (!props.class) return;
    const iCalendarEvents = createICalendarEvents([props.class]);

    await save({
      defaultPath: `${props.class.class_name}.ics`,
      filters: [
        {
          name: 'iCalendar',
          extensions: ['ics'],
  ...
  return (
    <div class={`fixed z-10 inset-0 overflow-y-auto flex justify-end`}>
      <div
        ref={drawerRef}
        class={`card bg-base-200 shadow-lg transform transition-transform duration-500 ${translateX()} max-w-md w-full`}
      >
        <div class="p-6">
          ...
          <a
            onMouseDown={() =>
              openInBrowser(props.class?.detailed_link)
            }
            class="cursor-pointer opacity-50 text-secondary"
          >
            Se På Lectio
          </a>
          <div class="mt-4 space-y-2">
            ...
            <p>
              <span class="font-bold">Klassenavn:</span>{' '}
              {props.class?.class_name}
            </p>
            <p>
              <span class="font-bold">Lærer:</span>{' '}
              {props.class?.teacher}
            </p>
            <p>
              <span class="font-bold">Lokale:</span>{' '}
              {props.class?.room}
            </p>
            <p>
              <span class="font-bold">Beskrivelse:</span>{' '}
            ...
            <p>
              ...
              {getFullDayName(props.class?.day)}
            </p>
            <p>
              <span class="font-bold">Lektier:</span>{' '}
              {props.class?.homework}
            </p>
            <p>
              <span class="font-bold">Ressourcer:</span>{' '}
              {props.class?.resources}
            </p>
            <p>
              <span class="font-bold">Noter:</span>{' '}
              {props.class?.notes}
            </p>
            <button class="btn btn-primary" onMouseDown={downloadClassSchedule}>
              Tilføj Lektion til Kalender
            </button>
          </div>
        </div>
      </div>
